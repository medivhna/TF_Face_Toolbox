...
class ResNeXt(object):
    def __init__(self, num_layers=50, num_card=32, data_format='NCHW', name='ResNeXt'):
    ...
    def resBlock(self, x, num_outputs, stride=1, activation_fn=tf.nn.relu, normalizer_fn=layers.batch_norm, scope=None):
        ...
        with tf.variable_scope(scope, 'resBlock'):
            ...
            x = layers.conv2d(x, num_outputs, kernel_size=1, stride=1, activation_fn=None, normalizer_fn=None)
    ...
    def __call__(self, inputs, is_training=False, reuse=False):
        with tf.variable_scope(self.name, reuse=reuse):
            with arg_scope([layers.batch_norm], scale=True, fused=True, data_format=self.data_format, is_training=is_training):
                with arg_scope([layers.conv2d], activation_fn=tf.nn.relu, 
                    ...
                    with tf.variable_scope('conv1'):
                        ...
                        net = layers.max_pool2d(net, kernel_size=3, stride=2, padding='SAME', data_format=self.data_format)
                    ...
                    pre_logits = layers.fully_connected(net, num_outputs=128, 
                                                        activation_fn=None, 
                                                        weights_regularizer=layers.l2_regularizer(0.0005))
